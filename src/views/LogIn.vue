    ...
    <div id="login" class="login" style="opacity: 1; display: block; transform: translateY(0px);">
        ...
        <div class="login-form">
          ...
          C-191.3,356.5-196.9,350.3-203.7,350.3L-203.7,350.3z"></path>
                ...
                <input v-model="uname" type="text" id="username_input" class="username-input" placeholder="请输入用户名">
            </div>
          ...
          l1.6-6.7l7.1-0.3l0.6-7.2l7.2-6.6c2.8,1.3,5.8,2,9.1,2c11.9,0,21.6-9.7,21.6-21.6C-169.9,357.4-179.6,347.8-191.5,347.8z"></path>
                ...
                <input v-model="upwd" type="password" id="password_input" class="password-input" placeholder="请输入密码">
            </div>
        </div>
        <div class="call-to-action">
            <button @click="login" id="login-button" type="button">登录</button>
            <p>没有账号? <router-link to="/user/signup">去注册</router-link></p>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            login(){
                ...
                var url = `http://127.0.0.1:3000/login?uname=${this.uname}&upwd=${this.upwd}`;
        ...
        mounted(){
            ...
            $("button").on("click", function () {
                if($("#username_input").val()=='')
                    $("#username_input").velocity("callout.shake");
                if($("#password_input").val()=='')
                    $("#password_input").velocity("callout.shake");
            });
        }
    }
</script>
